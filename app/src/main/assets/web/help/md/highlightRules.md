...
> **九宫格调整**：适配方式选"九宫格"且已设置背景图时，编辑页出现"九宫格调整"入口。弹窗内预览图叠加四条红色分割线，下方滑条按百分比微调分割线位置，实时预览、确定后生效。原先对 .9.png 的自动适配（自动检测透明留白并外扩）已移除，改由用户在此手动调整。弹窗左下角有「重置」，点一下把弹窗内所有取值（分割比例、外扩策略、四边间距）恢复默认，仍需点「确定」才会写入规则。
