...
编辑页与规则列表卡片都会按 `sampleText` 实时预览当前样式。编辑页的预览卡片位于滚动内容最底部，调参时常被输入区挡住，因此**没滑到底时会在底部悬浮一份同步预览**（标题为「预览效果（悬浮）」）；滑到底、原卡片已经露出来之后，悬浮层自动收起，改用原来的卡片，不会出现两份预览。

预览按控件**实际可用宽度**重新断行，所以命中字距、命中行行距这类会影响排版宽高的取值在预览里与阅读页一致。

#### 命中字距（letterSpacingBefore / letterSpacingAfter）

编辑页「命中字距（px）」卡片，`命中前` / `命中后` 分别控制命中段与**左侧/右侧相邻文字**之间留出的空白，单位 px（滑块一格 0.1px，范围 0~120）。默认 0 表示不留白。

| 参数 | 默认 | 含义 |
|---|------|------|
| `letterSpacingBefore` | 0 | 命中段左侧留白（px） |
| `letterSpacingAfter` | 0 | 命中段右侧留白（px） |

- 留白**落在命中段外侧**：命中段内部字符间距不变，只有段首字符拿到左侧留白、段尾字符拿到右侧留白，所以整段文字不会被从内部撑开；命中段被换行拆开时，留白只出现在整段最外两端，续行的行首/行尾不再额外留白。
- 留白是**真实排版宽度**：断行会把它算进去（行尾命中不会把整行挤回去），两端对齐与标题居中/右对齐也会把它算进行宽，所以高亮文字不会从右侧溢出、也不会被越界兜底反向压扁（预览与阅读页的换行位置因此一致）。
...
- 命中行行距**计入翻页判断**：命中行正好落在页底时会正常翻页，不会因为行距把内容推到版面外（正文逐列排版路径）。
...
- **数组顺序即列表顺序**：拖动排序后按新顺序整体写回，不额外记录排序字段

### 2.7 规则列表操作

| 操作 | 位置 | 说明 |
|---|---|---|
| 编辑 | 点卡片任意位置 / 点「编辑」 | 打开规则编辑弹窗 |
| 拖动排序 | **长按卡片**（预览区也可以） | 卡片带触感拾起、放大并加阴影，拖到位松手即写入新顺序 |
| 更多 | 点卡片右下角「更多」 | 居中弹出操作菜单：编辑 / 导出 / 分享 / 删除 |
| 启用/停用 | 卡片右上角开关 | 立即生效并持久化 |

- **分组筛选下的拖动**：只重排当前分组占用的那些位置，其他分组的规则顺序保持不动；顺序没有变化时不写存储、不刷新阅读页。
- 长按已用于拖动排序，所以原来的"长按弹操作菜单"改到「更多」按钮；删除也一并收进菜单里，卡片上只保留「编辑」和「更多」两个入口。
